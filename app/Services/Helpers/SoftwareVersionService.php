...
class SoftwareVersionService
{
    public function latestPanelVersion(): string
    {
        ...
        return cache()->remember($key, now()->addMinutes(config('panel.cdn.cache_time', 60)), function () {
            try {
                $response = Http::timeout(5)->connectTimeout(1)->get('https://api.github.com/repos/lionpanel-dev/panel/releases/latest')->throw()->json();
    ...
    public function latestWingsVersion(): string
    {
        ...
        return cache()->remember($key, now()->addMinutes(config('panel.cdn.cache_time', 60)), function () {
            try {
                $response = Http::timeout(5)->connectTimeout(1)->get('https://api.github.com/repos/lionpanel-dev/wings/releases/latest')->throw()->json();
